...
const App = () => {
 ...
 const [plantData, setPlantData]=useState({
  ...
  const handleButtonClick = async () => {
    ...
    try {
      const response = await fetch(`https://big-flzf.onrender.com/${buttonState ? 'true' : 'false'}`, {
        method: 'POST',
        headers: {
  ...
  const handleFanClick = async () => {
    ...
    try {
      const response = await fetch(`https://big-flzf.onrender.com/${fanState}`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  const handleLightClick = async () => {
    ...
    try {
      const response = await fetch(`https://big-flzf.onrender.com/${lightState}`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  const handleMotorClick = async () => {
    ...
    try {
      const response = await fetch(`https://big-flzf.onrender.com/${motorState}`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
